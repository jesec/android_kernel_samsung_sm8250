...
#include <net/oppo/oppo_apps_power_monitor.h>
#include <linux/tcp.h>
#include <net/tcp.h>
...
static void print_report_info(struct app_monitor_ul_report_msg_st *report_msg){
    ...
    printk("[app_monitor]: report_info:beat_app_count=%u, push_app_count=%u, traffic_app_count=%u\n",
    report_msg->beat_app_count, report_msg->push_app_count, report_msg->traffic_app_count);
    ...
    for(i = 0; i < report_msg->traffic_app_count; i++){
        printk("[app_monitor]:report_info:traffic:i=%d,uid=%u,total_packet=%llu,send_packet=%llu,recv_packet=%llu\n",
...
void oppo_app_monitor_fill_beat_app_info(struct app_monitor_ul_report_msg_st *report_msg)
{
    int  i, j, k;
    ...
    for(i = 0; i < app_num; i++){
        ...
        for(j = 0; j < TOP_APP_NUM;  j++){
            ...
            if(!min_period || min_period > top_app_beat_info.app_beat_info[j].beat_period){
                min_period = top_app_beat_info.app_beat_info[j].beat_period;
                k = j;
            }
        }
...
void oppo_app_monitor_fill_push_app_info(struct app_monitor_ul_report_msg_st *report_msg)
{
    int  i, j, k;
    ...
    for(i = 0; i < app_num; i++){
        ...
        for(j = 0; j < TOP_APP_NUM;  j++){
            ...
            if(!min_period || min_period > top_app_push_info.app_push_info[j].push_period){
                min_period = top_app_push_info.app_push_info[j].push_period;
                k = j;
            }
        }
...
void oppo_app_monitor_fill_traffic_app_info(struct app_monitor_ul_report_msg_st *report_msg)
{
    int  i, j, k;
...
void oppo_app_monitor_send_app_info_report(void)
{
    int ret;
    char msg_buf[1024] = {0};
    ...
    if(!app_monitor_start || before((u32)now_tv.tv_sec , (u32)(REPORT_INTERVAL + last_report_tv.tv_sec))){
        /*printk("[app_monitor]:oppo_app_monitor_send_app_info_report,now_tv=%lld,last_tv=%lld\n",
    ...
    oppo_app_monitor_fill_push_app_info(report_msg);
    oppo_app_monitor_fill_beat_app_info(report_msg);
    oppo_app_monitor_fill_traffic_app_info(report_msg);
...
static void apps_monitor_report_timer_function(void){
    oppo_app_monitor_send_app_info_report();
...
static void report_timer_del(void){
    ...
    del_timer(&apps_monitor_report_timer);
}

static void oppo_app_monitor_start(void)
{
    int i;
...
static void oppo_app_monitor_stop(void)
{
    unsigned long flags;
    write_lock_irqsave(&app_power_monitor_lock, flags);
...
static inline u32 oppo_app_monitor_find_app_traffic_index(u32 uid_val)
{
    u32 index;
...
void oppo_app_monitor_update_top_trafffic_app(struct app_monitor_app_info_st *app_info)
{
    ...
    app_index = oppo_app_monitor_find_app_traffic_index(app_uid);
    ...
    for(i = 0; i < TOP_APP_NUM; i++){
        ...
        else if(top_app_traffic_info.app_traffic_info[min_index]. total_bytes > top_app_beat_info.app_beat_info[i].total_bytes){
            min_index = i;
        }
    }
...
static  inline u32 oppo_app_monitor_find_app_beat_index(u32 uid_val)
{
    u32 index;
...
void oppo_app_monitor_update_top_beat_app(struct app_monitor_app_info_st *app_info)
{
    ...
    app_index = oppo_app_monitor_find_app_beat_index(app_uid);
    //app record exsit, just update it
    if(app_index < TOP_APP_NUM){
...
static  inline u32 oppo_app_monitor_find_app_push_index(u32 uid_val)
{
    u32 index;
...
static void oppo_app_monitor_update_top_push_app(struct app_monitor_app_info_st *app_info)
{
    ...
    app_index = oppo_app_monitor_find_app_push_index(app_uid);
    //app record exsit, just update it
    if(app_index < TOP_APP_NUM){
...
static struct app_monitor_app_info_list_node_st* oppo_app_monitor_get_app_info_by_uid(u32 uid)
{
...
void oppo_app_monitor_update_flow_stat(struct app_monitor_app_info_list_node_st *app_info_node,
...
void oppo_app_monitor_send_beat_alarm(u32 uid_val, u32 period)
{
    int ret;
    char msg_buf[1024] = {0};
...
void oppo_app_monitor_send_push_alarm(u32  uid_val, u32 period)
{
    int ret;
    char msg_buf[1024] = {0};
...
void oppo_app_monitor_send_traffic_alarm(struct app_monitor_app_info_st *app_info)
{
    int ret;
    char msg_buf[1024] = {0};
...
void oppo_app_monitor_update_app_info(struct sock *sk, const struct sk_buff *skb, int send)
{
    struct tcp_sock *tp = tcp_sk(sk);
    u32 uid_val;
    u32 hash_index;
    struct app_monitor_app_info_list_node_st *app_info_node = 0;
    u32 sk_samp_time; //unit:second
    ...
    write_lock_irqsave(&app_power_monitor_lock, flags);
    if(!app_monitor_enable || !skb->len || uid_val < 10000){
        goto exit_func;
    }

    app_info_node =  oppo_app_monitor_get_app_info_by_uid(uid_val);
    if(!app_info_node){
    ...
    oppo_app_monitor_update_flow_stat(app_info_node, skb, send);
    oppo_app_monitor_update_top_trafffic_app(&app_info_node->app_info);

    //do beat check ralative operation
    if(send){
        if(!tp->tcp_last_send_time){
            goto exit_func;
        }

        if(after(now_sec, tp->tcp_last_active_time + PERIOD_CHECK_TIME)){
            sk_samp_time  = now_sec - tp->tcp_last_active_time;
            //update sk beat period and beat count
            tp->tcp_beat_period = (tp->tcp_beat_period * tp->tcp_beat_count + sk_samp_time) / (tp->tcp_beat_count + 1);
            tp->tcp_beat_count++;
        }
        else{
            goto exit_func;
        ...
        if(app_samp_time >= PERIOD_CHECK_TIME){
            ...
            if(app_info_node->app_info.beat_count > 3){
                oppo_app_monitor_update_top_beat_app(&app_info_node->app_info);
            }
            ...
            if(app_info_node->app_info.beat_count % ALARM_DETECT_COUNT == 0){
                ...
                if(app_info_node->app_info.beat_alarm_start_time && time_gap < ALARM_DETECT_PERIOD){
                    oppo_app_monitor_send_beat_alarm(app_info_node->app_info.uid_val, period);
                }
        ...
        if(!tp->tcp_last_active_time){
            goto exit_func;
        }

        if(after(now_sec, tp->tcp_last_active_time + PERIOD_CHECK_TIME)){
            sk_samp_time  = now_sec - tp->tcp_last_active_time;
            //update sk push period and push count
            tp->tcp_push_period = (tp->tcp_push_period * tp->tcp_push_count + sk_samp_time) / (tp->tcp_push_count + 1);
            tp->tcp_push_count++;
        }
        else{
            goto exit_func;
        ...
        if(app_samp_time > PERIOD_CHECK_TIME){
            ...
            if(app_info_node->app_info.push_count > 3){
                oppo_app_monitor_update_top_push_app(&app_info_node->app_info);
            }
            ...
            if(app_info_node->app_info.push_count % ALARM_DETECT_COUNT == 0){
                ...
                if(app_info_node->app_info.push_alarm_start_time && time_gap < ALARM_DETECT_PERIOD){
                    oppo_app_monitor_send_push_alarm(app_info_node->app_info.uid_val, period);
                }
...
exit_func:
    tp->tcp_last_active_time = now_sec;
    if(app_info_node)
        app_info_node->app_info.last_active_time = now_sec;
    if(send){
        tp->tcp_last_send_time = now_sec;
        if(app_info_node)
...
EXPORT_SYMBOL(oppo_app_monitor_update_app_info);
...
int app_monitor_dl_ctl_msg_handle(struct nlmsghdr *nlh)
{
    ...
    if(dl_ctl_msg->start_flag){
        oppo_app_monitor_start();
    }else{
        oppo_app_monitor_stop();
    }
...
static int proc_app_set_beat_alarm_period(struct ctl_table *ctl, int write, void __user *buffer, size_t *lenp, loff_t *ppos)
{
    ...
    if (write && ret == 0){
        ...
        oppo_app_monitor_send_beat_alarm(top_app_beat_info.app_beat_info[index].uid_val, (u32)app_beat_alarm_period);
...
static int proc_app_set_push_alarm_period(struct ctl_table *ctl, int write, void __user *buffer, size_t *lenp, loff_t *ppos)
{
    ...
    if (write && ret == 0){
        ...
        oppo_app_monitor_send_push_alarm(top_app_push_info.app_push_info[index].uid_val, (u32)app_push_alarm_period);
...
static int  oppo_app_power_monitor_sysctl_init(void)
{
    app_power_monitor_table_hrd = register_net_sysctl(&init_net, "net/oppo_app_power_monitor", app_power_monitor_sysctl_table);
...
void oppo_app_power_monitor_init(void)
{
    ...
    report_timer_init();
    oppo_app_power_monitor_sysctl_init();
    ...
    memset(&top_app_push_info, 0, sizeof(struct app_monitor_top_app_beat_info_st));
    ...
    write_unlock_irqrestore(&app_power_monitor_lock, flags);
    printk("[app_monitor]:oppo_app_power_monitor_init\n");
    return;
}
