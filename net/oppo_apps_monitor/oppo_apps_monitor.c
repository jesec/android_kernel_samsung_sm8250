...
//the dual sta feature, the wlan/cell index is changed in oppo_sla.c
...
static int rrt_period_report_timer = 30; //30 sec
...
int apps_monitor_netlink_send_to_user(int msg_type, char *payload, int payload_len)
{
	...
	if (!apps_monitor_netlink_pid) {
		printk("oppo_apps_monitor: apps_monitor_netlink_send_to_user, can not unicast skbuff, apps_monitor_netlink_pid=0\n");
		return -1;
	}
	...
	if (skbuff == NULL) {
		printk("oppo_apps_monitor: apps_monitor_netlink_send_to_user, skbuff alloc_skb failed\n");
		return -1;
	}
	...
	if (nlh == NULL) {
		printk("oppo_apps_monitor: apps_monitor_netlink_send_to_user, nlmsg_put failaure\n");
		nlmsg_free(skbuff);
		return -1;
	}
	...
	if(apps_monitor_netlink_pid){
		...
		printk(KERN_ERR "oppo_apps_monitor: apps_monitor_netlink_send_to_user, can not unicast skbuff, apps_monitor_netlink_pid=0\n");
		kfree_skb(skbuff);
	}

	if(ret < 0){
		printk(KERN_ERR "oppo_apps_monitor: apps_monitor_netlink_send_to_user, can not unicast skbuff, ret = %d\n", ret);
		return -1;
	}
...
#if 0
	printk("oppo_apps_monitor: statistics_monitor_apps_rtt_via_uid, if_index = %d, rtt = %d\n", if_index, rtt);
#endif
...
{
	apps_monitor_table_hrd = register_net_sysctl(&init_net, "net/oppo_apps_monitor",
...
static int apps_monitor_set_android_pid(struct sk_buff *skb)
{
	...
	printk("oppo_apps_monitor: apps_monitor_netlink_set_android_pid pid=%d\n",apps_monitor_netlink_pid);
	return 0;
}
...
static int apps_monitor_set_apps_uid(struct nlmsghdr *nlh)
{
	...
	if (apps_uid_num >= MONITOR_APPS_NUM_MAX) {
		printk("oppo_apps_monitor: the input apps_uid_num is bigger than MONITOR_APPS_NUM_MAX! \n");
		return -EINVAL;
	}
	...
	for (index = 0; index < apps_uid_num; ++index) {
		...
		if (apps_monitor_debug) {
			printk("oppo_apps_monitor: apps_monitor_netlink_set_apps_uid, g_monitor_apps_table[%d].app_uid = %d \n", index, g_monitor_apps_table[index].app_uid);
		}
	}
...
static int apps_monitor_set_rtt_thred(struct nlmsghdr *nlh)
{
	...
	if (rtt_thred_num > sizeof(rtt_params_thred) / sizeof(u32)) {
		printk("oppo_apps_monitor: the input rtt_thred_num is bigger than except! the input rtt_thred_num=  %d \n", rtt_thred_num);
		return -EINVAL;
	}
...
static void print_apps_rtt_record(void) {
	...
	for (i = 0; i < g_monitor_apps_num; ++i) {
		for(j = 0; j < IFACE_NUM_MAX; ++j) {
			printk("oppo_apps_monitor: print_apps_rtt_record, the uid = %d, the if_index = %d, RTT = %llu:%llu:%llu:%llu:%llu:%llu\n",
...
static void print_dev_rtt_record(void) {
	...
	for(i = 0; i < IFACE_NUM_MAX; ++i) {
		printk("oppo_apps_monitor: print_dev_rtt_record, the if_index = %d, RTT = %llu:%llu:%llu:%llu:%llu:%llu\n",
												i,
...
#define MAX_RTT_MSG_LEN (2048)
	...
	if ((rtt_params_size + int_size) * IFACE_NUM_MAX * g_monitor_apps_num > MAX_RTT_MSG_LEN) {
		printk("oppo_apps_monitor: apps_monitor_report_apps_rtt_to_user, the RTT Msg is too big, len = %0x \n",
	...
	switch (if_index) {
		...
		case ALL_IF_INDEX:
			...
			break;
		default:
			printk("oppo_apps_monitor: apps_monitor_report_apps_rtt_to_user, the if_index is unvalue! \n");
			return -EINVAL;
	}
	...
	if (ret == 0) {
		...
		printk("oppo_apps_monitor: apps_monitor_netlink_send_rtt_to_user fail! \n");
	}
...
#define MAX_DEV_RTT_MSG_LEN (256)
	...
	if (rtt_params_size * IFACE_NUM_MAX > MAX_DEV_RTT_MSG_LEN) {
		printk("oppo_apps_monitor: apps_monitor_report_dev_rtt_to_user, the RTT Msg is too big, len = %0x \n",
	...
	if (ret == 0) {
		...
		printk("oppo_apps_monitor: apps_monitor_report_dev_rtt_to_user fail! \n");
	}
...
static int apps_monitor_netlink_nlmsg_handle(struct sk_buff *skb, struct nlmsghdr *nlh, struct netlink_ext_ack *extack)
{
	...
	if (apps_monitor_debug) {
		printk("oppo_apps_monitor: apps_monitor_netlink_recv_handle, the nlh->nlmsg_type = %0x \n", nlh->nlmsg_type);
	}
...
static int apps_monitor_netlink_init(void)
{
	...
	apps_monitor_netlink_sock = netlink_kernel_create(&init_net, NETLINK_OPPO_APPS_MONITOR, &cfg);
...
static int __init oppo_apps_monitor_init(void)
{
	int ret = 0;
	...
	if (ret < 0) {
		printk("oppo_apps_monitor: oppo_apps_monitor_init module failed to init netlink.\n");
	} else {
		printk("oppo_apps_monitor: oppo_apps_monitor_init module init netlink successfully.\n");
	}
	...
	if (is_need_period_timer()) {
       ...
       oppo_app_power_monitor_init();
	return ret;
}

static void __exit oppo_apps_monitor_fini(void)
{
	...
	oppo_app_power_monitor_fini();
}

module_init(oppo_apps_monitor_init);
module_exit(oppo_apps_monitor_fini);
