...
static inline void
blk_update_latency_hist(struct io_latency_state *s, u_int64_t delta_us, unsigned int length)
{
	int i;
	...
	}else if( length > CHUNCK_SIZE_8K && length <= CHUNCK_SIZE_32K){
      s->latency_y_axis[i].chunk_8_32k_count++;
	}else if( length > CHUNCK_SIZE_32K && length <= CHUNCK_SIZE_64K){
      s->latency_y_axis[i].chunk_32_64k_count++;
	}else if( length > CHUNCK_SIZE_64K && length <= CHUNCK_SIZE_128K){
      s->latency_y_axis[i].chunk_64_128k_count++;
	}else if( length > CHUNCK_SIZE_128K && length <= CHUNCK_SIZE_256K){
