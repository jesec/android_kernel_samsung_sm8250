...
#ifdef VENDOR_EDIT
/*JianfengQiu@PSW.MM.AudioDriver.ADSP.2434874, 2019/11/26, Add for workaround fix adsp stuck issue*/
...
#ifdef VENDOR_EDIT
/*Zhao.Pan@PSW.MM.AudioDriver.Machine.2595926, 2019/12/3,
 *CR#2515155 add new clk state and lock for codec clk cmds*/
...
struct afe_ctl {
	...
	#ifdef VENDOR_EDIT
	/*Le.Li@PSW.MM.AudioDriver.ADSP.2620782, 2019/11/27, Add for Synchronize afe apr send pkt commands*/
	...
	#ifdef VENDOR_EDIT
	/*Jianfeng.Qiu@PSW.MM.AudioDriver.ADSP.2480784, 2019/10/28, Add for adsp crash workaround*/
	bool lpi_enabled;
	#endif /* VENDOR_EDIT */
				...
				unsigned long action, void *data)
{
	#ifdef VENDOR_EDIT
	/*Jianfeng.Qiu@PSW.MM.AudioDriver.ADSP.2480784, 2019/10/28, Add for adsp crash workaround*/
	int port_index = 0, island_mode = 0;

	for (port_index = IDX_AFE_PORT_ID_VA_CODEC_DMA_TX_0;
		port_index <= IDX_AFE_PORT_ID_VA_CODEC_DMA_TX_2; ++port_index) {
		afe_get_island_mode(port_index, &island_mode);
		if (island_mode)
			break;
	}
	#endif /* VENDOR_EDIT */
	...
	case SWR_WAKE_IRQ_REGISTER:
		#ifndef VENDOR_EDIT
		/*Jianfeng.Qiu@PSW.MM.AudioDriver.ADSP.2480784, 2019/10/28, Modify for adsp crash workaround*/
		afe_send_cmd_wakeup_register(data, true);
		#else /* VENDOR_EDIT */
		if (island_mode) {
			afe_send_cmd_wakeup_register(data, true);
			this_afe.lpi_enabled = true;
		}
		#endif /* VENDOR_EDIT */
		break;
	case SWR_WAKE_IRQ_DEREGISTER:
		...
		#else /* VENDOR_EDIT */
		if (this_afe.lpi_enabled) {
			afe_send_cmd_wakeup_register(data, false);
			this_afe.lpi_enabled = false;
		}
		#endif /* VENDOR_EDIT */
		break;
	default:
...
static int afe_send_cmd_port_start(u16 port_id)
{
	...
	if (ret)
	#ifndef VENDOR_EDIT
	//Zhao.Pan@PSW.MM.AudioDriver.Adsp.2595926 , 2019/11/22, Add panic for bug 2595926.
	...
	#else
	{
		pr_err("%s: AFE enable for port 0x%x failed %d\n", __func__,
		       port_id, ret);
		if ((0x400e == port_id) && (-110 == ret)) {
			pr_err("%s():force restart adsp for bug 2595926\n",  __func__);
			result = subsystem_restart("adsp");
			pr_err("%s():force restart adsp result:%d\n",  __func__, result);
		}
	}
	#endif /* VENDOR_EDIT */
...
int afe_set_lpass_clk_cfg(int index, struct afe_clk_set *cfg)
{
	...
	ret = q6afe_svc_pack_and_set_param_in_band(index, param_hdr,
						   (u8 *) cfg);
	#ifndef VENDOR_EDIT
	/*JianfengQiu@PSW.MM.AudioDriver.ADSP.2434874, 2019/11/26, Modify for workaround fix adsp stuck issue*/
	if (ret < 0)
