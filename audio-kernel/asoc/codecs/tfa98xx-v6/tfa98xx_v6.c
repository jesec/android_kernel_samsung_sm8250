...
#ifndef VENDOR_EDIT
/*Jianfeng.Qiu@PSW.MM.AudioDriver.SmartPA, 2019/08/09,
 *Modify for multi-project fw name.
 */
	...
	if (tfa98xx->tfa->dev_idx == 0) {
		...
		*speakerImpedance = tfa_get_calibration_info_v6(tfa98xx->tfa, 0);
		...
		list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
			...
			if(imp == 0) {
				pr_info("%s:  set the default 7.399 ohm to MTP, due to SPK was damage\n", __func__ );
			     ...
			     const struct i2c_device_id *id)
{
	...
	#ifdef VENDOR_EDIT
	/*Jianfeng.Qiu@PSW.MM.AudioDriver.SmartPA, 2019/08/09, Add for multi-project fw name*/
