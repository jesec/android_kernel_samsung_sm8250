...
#define pr_debug pr_info
	...
	while (!is_spl_hs) {
		...
		output_mv = wcd_measure_adc_once(mbhc, MUX_CTL_IN2P);
		#ifndef VENDOR_EDIT
		/*Jianfeng.Qiu@PSW.MM.AudioDriver.HeadsetDet, 2019/09/25, Modify for adc measure failed*/
...
#define pr_debug pr_info
	...
	#ifdef VENDOR_EDIT
	/*Jianfeng.Qiu@PSW.MM.AudioDriver.HeadsetDet, 2019/09/25, Add for invalid adc result*/
...
#define pr_debug pr_info
	...
	if ((plug_type == MBHC_PLUG_TYPE_HEADPHONE) &&
	    ...
	    if (mbhc->mbhc_cfg->swap_gnd_mic &&
			...
			//not headset, switch back to last connection state
...
correct_plug_type:
	...
	while (!time_after(jiffies, timeout)) {
		...
		output_mv = wcd_measure_adc_once(mbhc, MUX_CTL_IN2P);
		#ifdef VENDOR_EDIT
		/*Jianfeng.Qiu@PSW.MM.AudioDriver.HeadsetDet, 2019/09/25, Add for adc measure failed*/
		if (output_mv < 0) {
		...
		if (output_mv > hs_threshold) {
			...
			wrk_complete = true;
			#ifdef VENDOR_EDIT
			/*Jianfeng.Qiu@PSW.MM.AudioDriver.HeadsetDet.1033864, 2017/03/06, Add for HIGH_HPH loop detect.*/
			high_hph_count++;
			...
			if (plug_type != MBHC_PLUG_TYPE_GND_MIC_SWAP) {
				...
				#ifdef VENDOR_EDIT
				/*Jianfeng.Qiu@PSW.MM.AudioDriver.HeadsetDet, 2019/09/25, Add for quick report 3pole*/
				if ((mbhc->current_plug == MBHC_PLUG_TYPE_HEADSET)
					&& (!wcd_swch_level_remove(mbhc))) {
...
report:
	...
	#ifdef VENDOR_EDIT
	/*Jianfeng.Qiu@PSW.MM.AudioDriver.HeadsetDet.1042404, 2019/09/25, Add for remove report*/
...
void wcd_mbhc_adc_init(struct wcd_mbhc *mbhc)
{
	...
	#ifdef VENDOR_EDIT
	//Nan.Zhong@PSW.MM.AudioDriver.HeadsetDet, 2019/11/01, Add for headset detect.
