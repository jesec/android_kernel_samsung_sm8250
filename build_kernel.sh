...
if [[ ! -z "$4" ]]
then
KERNEL_MAKE_ENV="$KERNEL_MAKE_ENV LOCALVERSION=-$4 DEBUG_DEFCONFIG=vendor/release_defconfig"
fi

make -j$(nproc) -C $(pwd) O=$(pwd)/out $KERNEL_MAKE_ENV ARCH=arm64 CROSS_COMPILE=$BUILD_CROSS_COMPILE CROSS_COMPILE_COMPAT=$BUILD_CROSS_COMPILE_COMPAT LLVM=1 CLANG_TRIPLE=$CLANG_TRIPLE vendor/${MODEL}_${REGION}_${CARRIER}_defconfig || exit 1
