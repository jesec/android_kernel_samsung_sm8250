...
make -j$(nproc) -C $(pwd) O=$(pwd)/out $KERNEL_MAKE_ENV ARCH=arm64 CROSS_COMPILE=$BUILD_CROSS_COMPILE CROSS_COMPILE_COMPAT=$BUILD_CROSS_COMPILE_COMPAT CC=${KERNEL_LLVM_BIN}clang LD=${KERNEL_LLVM_BIN}ld.lld NM=${KERNEL_LLVM_BIN}llvm-nm AR=${KERNEL_LLVM_BIN}llvm-ar OBJCOPY=${KERNEL_LLVM_BIN}llvm-objcopy OBJDUMP=${KERNEL_LLVM_BIN}llvm-objdump CLANG_TRIPLE=$CLANG_TRIPLE vendor/${MODEL}_${REGION}_${CARRIER}_defconfig || exit 1
