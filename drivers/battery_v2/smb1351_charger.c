...
static void smb1351_charger_qc_detection_work(struct work_struct *work)
{
	...
	__pm_stay_awake(&charger->qc_wake_lock);
	...
	__pm_relax(&charger->qc_wake_lock);
}
	...
	const struct i2c_device_id *id)
{
	...
	wakeup_source_init(&charger->qc_wake_lock, "smb1351-qc-detection");
...
err_req_irq:
	wakeup_source_trash(&charger->qc_wake_lock);
err_create_wqueue:
