						...
						struct sm5705_charger_data *charger)
{
	...
	wakeup_source_init(&charger->softstart_wake_lock, "charger-softstart");
#endif
	wakeup_source_init(&charger->wpc_wake_lock, "charger-wpc");
	wakeup_source_init(&charger->afc_wake_lock, "charger-afc");
	wakeup_source_init(&charger->check_slow_wake_lock, "charger-check-slow");
