...
unsigned long fps_required_util = 0;
struct list_head head_list;
int fps_task_count = 0;
unsigned long us_frame_time = 0;
int g_fps = 0;
int fps_margin_percent = 30;
	...
	switch(cmd) {
		...
		*/
		...
		case PERF_MGR_PROCESS_KILL:
			if (copy_from_user(&target_tid, uarg, sizeof(int))){
				pr_err("[GPIS] :PERF_MGR_PROCESS_KILL: fail to copy data from user");
				return -EFAULT;
			}
			if(fps_task_count <= 1)
				break;
			...
			trace_printk("[GPIS] ::: Delete TID is %d, Current Total Task Count : %d \n", target_tid, fps_task_count);
			break;
		...
		*/
		case PERF_MGR_TASK_ADD:
			...
			if (fi != NULL) 	// task already exist on drawing task list
				break;
			...
			spin_unlock(&write_slock);
			fps_task_count++;
			trace_printk("[GPIS] ::: New Task Adding, Add Task's Tid : %d in Group %d, Current Total Task Count : %d \n", fi->orig_fps_info.tid, fps_info_val.group_id, fps_task_count);
			break;

		/*
		 * In case of drawing frames finished, F/W calls this command.
		 * It checks if it's jank situation, if so, calculating fps boosted util on target task.
		 */

		case PERF_MGR_FRAME_END:
			if (copy_from_user(&fps_info_val, uarg, sizeof(struct fps_info))) {
				pr_err("[GPIS] : PERF_MGR_FRAME_END: fail to copy data from user");
				return -EFAULT;
			}
			task = find_task_by_vpid(fps_info_val.tid);
			...
			list_for_each_entry_rcu(fi, &head_list, list){
					...
					if(fi->orig_fps_info.tid == fps_info_val.tid)
						target_fi = fi;
					...
					tmp_task = find_task_by_vpid(fi->orig_fps_info.tid);
					if (tmp_task != NULL && tmp_task->drawing_flag == task->drawing_flag)
						rn_sum += get_task_util(tmp_task);
			}
			rcu_read_unlock();

			if(target_fi == NULL){
				pr_err("[GPIS] PID %d could not be found in the task list. skip to calculate required util\n", fps_info_val.tid);
				break;
			}

			/* We can choose the maximum value between current calculated value and previous saved value. */
			prev_fps_util = target_fi->updated_fps_util;
			duration = (fps_info_val.boosting_lvl > BOOST_OFF) ? (us_frame_time * 1000) : fps_info_val.duration;
			new_fps_util = calc_fps_required_util(rn_sum, duration);

			if (new_fps_util > 0 && fps_info_val.boosting_lvl != BOOST_LOW){
				rcu_read_lock();
				list_for_each_entry_rcu(fi, &head_list, list){
						if (fi->orig_fps_info.group_id == task->drawing_flag){
							tmp_task = find_task_by_vpid(fi->orig_fps_info.tid);
							if (tmp_task != NULL) tmp_task->drawing_mig_boost = 1;
						}
				}
				rcu_read_unlock();
			}

			/* Once we can choose boosted values between new and prev, initialize update info */
			
			if (fps_info_val.boosting_lvl != BOOST_MID){
				target_fi->updated_fps_util = max(target_fi->updated_fps_util, new_fps_util );
			}else{
			...
			trace_printk("[GPIS] ::: Margin : %d, Frame_Hold_Cnt : %d, FPS : %d, Tid : %d, Mig_boost : %d, Group : %d, boosting_lvl : %d, Runnable Sum : %lu, Dur : %d, Calc : %lu, Booosted_Util : %lu \n", fps_margin_percent, hold_frame_count, g_fps, target_fi->orig_fps_info.tid, task->drawing_mig_boost, task->drawing_flag, fps_info_val.boosting_lvl, rn_sum, duration, new_fps_util, fps_required_util);
			break;
		default:
			break;
	}
...
static struct task_fps_util_info *get_target_task(pid_t tid){
	...
	if (list_empty(&head_list)) return NULL;
...
unsigned long get_max_fps_util(int group_id){
	...
	unsigned long max_fps_util = 0;

	if (fps_task_count <= 1 || list_empty(&head_list) ) return 0;

	rcu_read_lock();
		list_for_each_entry_rcu(fi, &head_list, list){
				if ( fi != NULL && fi->orig_fps_info.group_id == group_id && (fi->updated_fps_util > max_fps_util) )
					max_fps_util = fi->updated_fps_util;
		}
	rcu_read_unlock();
...
int panel_timing_changed_data_notify(struct notifier_block *nb, unsigned long event_type, void *data)
{
	struct panel_dms_data *dms_data = data;

	if (PANEL_EVENT_VRR_CHANGED == event_type)
	{
		// get fps information
...
static ssize_t fps_margin_percent_show(struct kobject *kobj, struct kobj_attribute *attr,
						char *buf){
		return sprintf(buf, "%d\n", fps_margin_percent);
}

static ssize_t fps_margin_percent_store(struct kobject *kobj, struct kobj_attribute *attr,
...
static ssize_t hold_frame_count_show(struct kobject *kobj, struct kobj_attribute *attr,
						char *buf){
		return sprintf(buf, "%d\n", hold_frame_count);
}

static ssize_t hold_frame_count_store(struct kobject *kobj, struct kobj_attribute *attr,
						const char *buf, size_t n)
{
...
static int __init perf_mgr_dev_init(void)
{
	...
	if (err){
		pr_err("[GPIS] Failed to create the perf_manager sysfs file in /sys/gpis \n");
		return err;
	}
