...
EXPORT_SYMBOL(rmnet_get_port_rtnl);
...
static void rmnet_dellink(struct net_device *dev, struct list_head *head)
{
	...
	if (!real_port->nr_rmnet_devs)
		qmi_rmnet_qmi_exit(real_port->qmi_info, real_port);
...
void *rmnet_get_rmnet_port(struct net_device *dev)
{
	...
	if (dev) {
		priv = netdev_priv(dev);
		return (void *)rmnet_get_port_rtnl(priv->real_dev);
	}
