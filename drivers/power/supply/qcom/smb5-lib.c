...
void oppo_set_otg_switch_status_default(bool value);//add for single charge file (move to qpnp-smb5 after merge) to static
...
/* tongfeng.Huang@BSP.CHG.Basic, 2018/12/07,  Add for suspending*/
	smblib_err(chg, "[oppo_chg_max], %s = %d (0x%02x)\n",
		   param->name, val_u, val_raw);
#endif
	smblib_dbg(chg, PR_REGISTER, "%s = %d (0x%02x)\n",
		   param->name, val_u, val_raw);
...
#ifdef VENDOR_EDIT
/* Wenping.ZHOU@BSP.TP.Init, 2019/10/28, Add for notify touchpanel status */
	if (vbus_rising) {
		switch_usb_state(1);
	} else {
		switch_usb_state(0);
...
bool oppo_ccdetect_check_is_gpio_tmp(struct oppo_chg_chip *chip)
{
	struct smb_charger *chg = NULL;
	int boot_mode = get_boot_mode();
...
void oppo_set_otg_switch_status_default(bool value) //add for single charge file (move to qpnp-smb5 after merge) to static
{
	...
	if(oppo_ccdetect_check_is_gpio_tmp(g_oppo_chip))
	{
