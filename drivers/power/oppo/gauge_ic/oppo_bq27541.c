...
static int bq27441_battery_full_param_write_cmd(struct chip_bq27541 *chip)
{
	...
	bq27541_i2c_txsubcmd_onebyte(0x60, (read_buf[0] + read_buf[1] + reg_data - 0x11 - 0x21 ));
	pr_err("%s 0x60 -->write [0x%02x]\n", __func__, (read_buf[0] + read_buf[1] + reg_data - 0x11 - 0x21 ));
	...
	bq27541_i2c_txsubcmd_onebyte(0x60, (read_buf[0] + read_buf[1] + reg_data - 0x00 - 0x6f ));
	pr_err("%s 0x60 -->write [0x%02x]\n", __func__,(read_buf[0] + read_buf[1] + reg_data - 0x01 - 0x6f ));
	pr_err("%s end\n", __func__);
	return 0;
}
...
static int bq27411_write_soc_smooth_parameter(struct chip_bq27541 *chip, bool is_powerup)
{
	...
	if(chip->battery_full_param)
		bq27441_battery_full_param_write_cmd(chip);
